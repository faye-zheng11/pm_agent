...
description: 用适合当前产品的模拟用户组走查新产品或迭代产品，读取想法、方案、PRD、截图、Figma 或 HTML Demo，发现理解、动机、情绪、信任、文化、无障碍和安全问题。
---
...
- `runtime/references/framework-catalog.md`：选择用户旅程地图或心智模型时的适用边界。
...
始终声明这是模拟评审，不是真实用户研究。根据项目生成用户组，不固定套用某一 Persona。已有产品迭代必须比较改前与改后，指出修复了什么、是否破坏原有任务、引入了什么新风险，以及需要什么真人验证。
...
每次结果都要填写 `frameworks_used`。最多选择两个本 Agent 适用框架；框架用于组织走查，不得把模拟用户反应写成真人证据。
